...
MD_HEAD = """**<p align="center">[zack's Blog](https://zackzhangkai.github.io)</p>**
...
def get_to_generate_issues(repo, dir_name, issue_number=None):
    ...
    to_generate_issues = [
        ...
        if int(i.number) not in generated_issues_numbers and has_publish_label(i)
    ]
    if issue_number:
...
def main(token, repo_name, issue_number=None, dir_name=BACKUP_DIR):
    ...
    for issue in to_generate_issues:
        # should have publish label
        if has_publish_label(issue):
...
def get_tags(issue):
    ...
    for i in issue.labels:
        if i.name != "publish":
            tags = tags.append(i.name)
    return tags

def save_issue(issue, me, dir_name=BACKUP_DIR):
    title = get_date(issue) + "_" + str(issue.number) + "_" + issue.title.replace(' ', '.')
